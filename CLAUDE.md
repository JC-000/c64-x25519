...
**One issue raised upstream, open: [contract#177](https://github.com/JC-000/c64-lib-contract/issues/177).** §1 requires the deprecated bare version exports to be TU-isolated, because "ld65 pulls in whole object members". §8.4 emits a second family of unprefixed, cross-library-identical exports — `LIB_PRECALC_<name>_{SIZE,REGION,SHARED}` — and nothing says where they may live; ours are in `src/lib_manifest.s`, the member §5 requires a consumer to import from. Measured: a consumer importing *only* two prefixed §5 equates from two libraries gets `ld65: Error: Duplicate external identifier: 'LIB_PRECALC_sqtab_SHARED'`; splitting the invocations into their own member links clean. `c64-nist-curves` already isolates its invocations in `src/precalc_manifest.s`, so the fleet is split on a point the contract does not decide. **Do not unilaterally move ours** — it is a member-set change in two archives and the entire value of the rule is that both sides do the same thing. Wait for the ruling.
...
The library is 10 ca65 `.o` modules (no `main.o`, which is the BASIC stub / test harness / region-guard TU — downstream supplies its own entry point and mirrors the guard; the guard was built for §6.7, retired at contract v1.0.0 and kept because the overrun it catches is real). Module layout in `src/`:
