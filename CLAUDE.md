...
**Four things this repo learned the hard way. They are cheap to state and expensive to rediscover.**
...
4. **A stated claim and the artifact it describes are never compared by anything — so check the claim against the recipe, not against your intent.** On 2026-09-07 this class produced **five filed issues (#134, #139, #143, #144, #145) and three more caught in review on PR #146**; the count is given with its basis because a lesson that says *count from the recipe* must not carry a number counted from a chosen list. It arrives by three distinct routes, and they take different fixes:
    - **Wrong on arrival** — the sentence states the intent while the recipe produces the result, both written in the same commit. `git log -S` confirms it for #134 (one commit added the `SHARED_REU_MUL_FETCH` matrix leg *and* the stub export that makes it unfailable) and #139 (one commit added the `.ifndef LIB_NO_BARE_EXPORTS` gate, the stub's gate, and the comment claiming the stub tracks a define **no target in this repo ever set** — `c64-wireguard` has been passing it to us for releases (#139), which is what made the gap invisible). *Fix:* build or link the configuration the claim describes; write fixtures to the **external** surface rather than to your own demand; and where a claim must be prose, generate it from the build — `lib-nobare`'s banner prints the actual `X25519_PROFILE` / `CONTRACT_DEFINES` / `CONTRACT_ZP_DEFINES`, so the graded configuration cannot be misstated.
    - **Drift by omission** — a list that was right when written and wrong later. #145 (`build-fillneg` created by a commit that never touched `clean` or `.gitignore`) and the two sweep-list findings on PR #146. This is the shape of #122's `src/fe25519.s` pair, which were correct when written and drifted as lines were inserted above them — the rest of #122 was route 1. *Fix:* derive the list from the recipe rather than restating it; `clean` now spells the nobare trees with their variables because a literal copy of a derived name is exactly what drifted.
    - **Never written** — no false sentence and no list, just an unrecorded boundary a reader infers wrongly. #144 (*"scope neither enforced nor documented"*) and #143, where nothing anywhere claimed the ZP-suppressed mode was covered — the boundary was simply unrecorded. *Fix:* state the scope at the site and make violating it fail with a named message.

    The rule extends to **relayed** claims, and the supervisor's own instance was one: the #139 fix's commit message (`64f7f0f`, where it survives verbatim) and PR #146's body said the explicit `X25519_PROFILE=` meant the check "can never grade a build under a different profile than it was built with" — written from an agent's report rather than from the recipe, and false, since no failing case exists for the implicit form. Route 1 again, and the supervisor's own. A number or a "cannot" arriving in a report must be re-derived from the artifact before it reaches a commit message, a PR body or this file. A ninth instance — the `lib-nobare` banner's own earlier coverage comment, which claimed two consumer configurations and built neither — is deliberately left out of the count: its false text never landed in a commit, so it is not artifact-checkable, and applying the rule to the lesson's own enumeration is the point.

    Adjacent but **not** this class: a check that passes while examining nothing (#121, #133, `2ba58bb`'s roster guard). There the claim is true of the artifact and the artifact was never reached — that is lesson 2 and the gate's first condition, and it has a different fix.
...
The gate **sites** are deliberately not listed here or in the Makefile. They live as checked data in `tools/check_gate_citations.py`, verified by `make lib-verify-citations` on every `lib-verify`. That is issue #122's fix: the Makefile used to carry thirteen hand-maintained `file:line` citations and **nine pointed at blank lines, prose comments or ordinary instructions** — the `src/fe25519.s` pair off by one and off by nine, i.e. correct when written and drifted as lines were inserted above. Nothing read them, so nothing caught it. Prose holding no line numbers cannot drift, and the numbers that remain now fail the build that moves them.
