...
| L31c | src/x25519_init.s: reu_mul_init ×5 (2 inline + 3 in `X25519_DBL_STASH_ROW`, owner builds), x25519_sqr_tables_init ×4 (1 row fetch + 3 in `X25519_DBL_STASH_ROW`, §8.2 deferral builds at `SQR_DMA_K > 0` only), reu_probe ×4 | branch | low | clean | **v0.12.0, boot only; x25519_sqr_tables_init added for #159.** `REU_SETTLE` after every boot-time execute site. No secret data exists at these sites (public table enumeration and derivation from the provider's public table, sentinel round-trip), and none runs during a scalarmult — no network-observable exposure, catalogued for completeness under the "every execute site" rule. `reu_probe` folds the sticky `x25519_reu_fault` byte into its C=0 return |
...
> **Superseded mechanism (#134):** DMA #1 no longer delegates to
