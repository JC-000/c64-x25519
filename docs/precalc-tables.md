...
invocations in `src/precalc_manifest.s`** (the §6.1 isolated TU that holds
nothing else). The two forms (this
...
od65 --dump-exports build-1764/lib/precalc_manifest.o   | grep _PRECALC_
od65 --dump-exports build-onchip/lib/precalc_manifest.o | grep _PRECALC_
```
...
`LIB_PRECALC_TABLE` calls in `src/precalc_manifest.s`
...
|---|---:|---|---|---|---|---|
| `sqtab` | 1024 | RAM | **all profiles** | `src/x25519_init.s` (sqtab_init), `src/mul_8x8.s`, `src/fe25519.s` | Shareable (§8.1 normative) | Two 512-byte byte tables (`sqtab_lo`, `sqtab_hi`) implementing the quarter-square identity `a*b = floor((a+b)^2/4) - floor((a-b)^2/4)`. Bit-identical to the sibling implementations in `c64-nist-curves` and `c64-ChaCha20-Poly1305`; canonical placement equate is `LIB_SHARED_SQTAB_BASE`. Already adopted per §8.1 (v0.6.0, PR #56). Present in every profile; under `lib-x25519-onchip` it is read on *every* product by the on-chip row generator, so it moves from warm to firmly hot there. |
...
| `x25519_mul38_lo_tab` | 256 | RODATA | **all profiles** | `src/data.s` (`mul38_lo_tab`) | Curve-specific (2^256 ≡ 38 mod p) | Low byte of `38·i`, i = 0..255. Read by `mul_by_38` and the reduction paths in `src/fe25519.s`; page-aligned so a secret `abs,y` index never crosses a page (CT invariant, `lderror`-asserted in `src/data.s`). |
| `x25519_mul38_hi_tab` | 256 | RODATA | **all profiles** | `src/data.s` (`mul38_hi_tab`) | Curve-specific (2^256 ≡ 38 mod p) | High byte of `38·i`. Pair of `mul38_lo_tab`; page-aligned so a secret `abs,y` index never crosses a page (CT invariant, `lderror`-asserted in `src/data.s`). |
| `x25519_sqr_lo` | 256 | RODATA | **all profiles** | `src/data.s` (`sqr_lo`) | x25519-private (generic content) | Low byte of `i²`, i = 0..255. Read by `fe25519_sqr`'s diagonal terms. Not §8.1 `sqtab` (quarter-squares); page-aligned so a secret `abs,y` index never crosses a page (CT invariant, `lderror`-asserted in `src/data.s`). |
| `x25519_sqr_hi` | 256 | RODATA | **all profiles** | `src/data.s` (`sqr_hi`) | x25519-private (generic content) | High byte of `i²`. Pair of `sqr_lo`; page-aligned so a secret `abs,y` index never crosses a page (CT invariant, `lderror`-asserted in `src/data.s`). |
| `x25519_a24_b0` | 256 | RODATA | **all profiles** | `src/data.s` (`a24_b0`) | Curve-specific (a24 = 121665) | Byte 0 of `121665·i`, i = 0..255. Read by `fe25519_mul_a24`; page-aligned so a secret `abs,y` index never crosses a page (CT invariant, `lderror`-asserted in `src/data.s`). |
| `x25519_a24_b1` | 256 | RODATA | **all profiles** | `src/data.s` (`a24_b1`) | Curve-specific (a24 = 121665) | Byte 1 of `121665·i`; page-aligned so a secret `abs,y` index never crosses a page (CT invariant, `lderror`-asserted in `src/data.s`). |
| `x25519_a24_b2` | 256 | RODATA | **all profiles** | `src/data.s` (`a24_b2`) | Curve-specific (a24 = 121665) | Byte 2 of `121665·i`; page-aligned so a secret `abs,y` index never crosses a page (CT invariant, `lderror`-asserted in `src/data.s`). |
| `x25519_a24_b3` | 256 | RODATA | **all profiles** | `src/data.s` (`a24_b3`) | Curve-specific (a24 = 121665) | Byte 3 of `121665·i` (0 or 1); page-aligned so a secret `abs,y` index never crosses a page (CT invariant, `lderror`-asserted in `src/data.s`). |

The eight `src/data.s` rows are named `x25519_<symbol>` rather than by the
bare symbol: they are private (`SHARED_NO`), and the bare
`LIB_PRECALC_<name>_*` triple is one namespace across every adopter, so an
unprefixed private name could collide with a sibling's in a composed link.
...
- Tables flagged `PRECALC_SHARED_YES` here are the ones whose
  ...
  `od65 --dump-exports build/lib/precalc_manifest.o | grep _PRECALC_<name>`
