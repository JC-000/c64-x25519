...
od65 --dump-exports build-1764/lib_version.o   | grep LIB_PRECALC
od65 --dump-exports build-onchip/lib_version.o | grep LIB_PRECALC
```
...
`LIB_PRECALC_TABLE` calls in `src/lib_version.s`
...
- `LIB_X25519_SHARED_PRIMITIVES` (`src/lib_version.s`) ORs in the
  ...
  manifests via the §8.0 double-ownership `.assert`.
