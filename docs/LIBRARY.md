...
| `SQTAB` MEMORY region | base equal to `LIB_SHARED_SQTAB_BASE` (default `$7800`), 1 KB reserved | **Named link error since v0.12.0-prep** (was fully silent): the §6.7 guard TU (`src/main.s`, never archived) asserts `__MAIN_LAST__ <= LIB_SHARED_SQTAB_BASE` (image overrun) *and* `__SQTAB_START__ = LIB_SHARED_SQTAB_BASE` / `__SQTAB_SIZE__ >= LIB_X25519_PRECALC_sqtab_SIZE` (region-agreement superset beyond the SPEC minimum, which closes image-overrun only — a diverged region linked clean at base `0x2A00`, measured). Both fire as `lderror` with named messages. Consumers who write their own cfg mirror the same three asserts against their map (§6.7) — all three compare against a **published symbol** (`LIB_SHARED_SQTAB_BASE`, `LIB_X25519_PRECALC_sqtab_SIZE`), so a mirrored copy cannot drift from the library's declared geometry the way a hardcoded `1024` could; in the shipped cfg, growing MAIN past `$7800` was already a hard area-overflow error — the silent case was consumer-cfg-only. |
...
| `LIB_X25519_RESIDENT_BYTES` | `8547` default / `8381` for `lib-x25519-1764` / `8234` for `lib-x25519-onchip` | Approximate code + data + sqtab footprint that must remain CPU-resident. Dropped from 9209/8895 at the issue-#68 cold-segment split — the init-only code is now counted in `LIB_X25519_COLD_BYTES` (SPEC §5 disjoint partition; see §4.10). All three figures od65-measured; the onchip value has been measured-exact since the profile shipped in v0.8.0. v0.12.0 grew default/1764 by +93/+81 for the §8.2 v0.13.0 REU settle (§4.12); onchip touches no REU and is unchanged. Default +15 since, for `reu_fetch_doubled_row` issuing its own row DMA (#134; not present in 1764). Default and 1764 +26 since, for `reu_fetch_mul_row` writing every FETCH register itself (#164) |
...
| `LIB_X25519_RESIDENT_BYTES` | `8547` | `8381` |
...
- The host program calls the canonical `reu_mul_tables_init` from
  another library, which builds only the shared a*b pair (base,
  base+1).
- **With `SQR_DMA_K > 0` (the default) the host must then call
  `x25519_sqr_tables_init`**, before any c64-x25519 field op.
  `fe25519_sqr` also reads x25519's private doubled/carry banks
  (base+3..+5), which the canonical init does not build; this entry
  derives them from the provider's table (same bytes an owner build's
  `reu_mul_init` writes). Skip it and every square is wrong. The
  1764 variant (`SQR_DMA_K = 0`) reads no private banks and does not
  export it.

```
        jsr mul_tables_init          ; §8.1
        jsr reu_mul_tables_init      ; §8.2 provider: banks base, base+1
        jsr x25519_sqr_tables_init   ; x25519 private: base+3..+5
```

`tools/test_shared_reu_deferral.py` runs this sequence in VICE against a
functional stand-in provider and checks `fe25519_sqr` and the RFC 7748
vectors against pyca/cryptography.
...
| `LIB_X25519_RESIDENT_BYTES` | `8547` | `8234` |
